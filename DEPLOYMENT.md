...
| `RESO_MOCK` | ✅ | `true` until GLVAR/Spark credentials are live and verified; then switch to `false` |
...
4. Approval typically takes 3–10 business days. Set `RESO_MOCK=true` in the interim to use seed data, then switch to `false` only after `/api/idx/verify` confirms the live feed is connected.
...
> **Important:** Vercel does not support environment variable substitution in `vercel.json` rewrite destinations. This repository already points at `https://donnasellslv-server.onrender.com`; if your Render service URL changes, update both rewrites before deploying.

Confirm `vercel.json` points at your Render service URL:
...
Commit and push any rewrite changes before deploying to Vercel.

### Pre-deployment checklist

- [ ] `vercel.json` — both rewrites point to the correct Render web service URL
- [ ] `app/index.html` — canonical URL and social links confirmed (`twitter:site` only if an X account exists)
- [ ] `app/index.html` — phone, license number, and social metadata confirmed
- [ ] `app/horses/index.html` — canonical URL and social links confirmed (`twitter:site` only if an X account exists)
- [ ] `app/horses/index.html` — phone, license number, and social metadata confirmed
- [ ] `app/robots.txt` — sitemap URL points to `https://www.donnasellslv.com/sitemap.xml`
- [ ] Render env — `SMTP_*`, `EMAIL_FROM`, `ADMIN_EMAIL`, `ADMIN_INITIAL_PASSWORD`, and optional `TURNSTILE_*` configured
- [ ] Frontend forms — `/v2/inquiries` and `/v2/newsletter/subscribe` respond successfully through Vercel rewrites
